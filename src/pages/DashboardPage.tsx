...
import { BookOutlined, CheckOutlined, ExperimentOutlined, FireOutlined, RightOutlined } from "@ant-design/icons";
import { PLAN } from "../data/plan";
import { todayStr, type StudyLog } from "../store";

type Destination = "current" | "challenge" | "training" | "memory" | "bestiary" | "showcase";
...
export default function DashboardPage({ onNavigate }: { onNavigate: (page: Destination) => void }) {
  ...
  const trainingDays = [...read<string>("study_tracker_skill_days"), ...read<string>("study_tracker_duck_days")];
  ...
  const states = [mainDoneToday, algorithmDays.includes(today), trainingDays.includes(today)];
  const completed = states.filter(Boolean).length;

  return <main className="page-shell dashboard-page">
    <section className="dashboard-hero"><div><span className="eyebrow">TODAY'S ADVENTURE</span><h1>{completed === 3 ? <><span className="title-line">三项试炼：卒</span><span className="title-line">勇者：原地升级</span></> : "勇者，请领取今日三项试炼"}</h1><p>不追求一次学很多，只完成今天最重要的主线、打怪和技能训练。</p></div><Progress type="circle" percent={Math.round(completed / 3 * 100)} format={() => `${completed}/3`} /></section>
    <div className="today-grid">
      ...
      <Card className={states[2] ? "today-card is-complete" : "today-card"}><Tag color="blue">勇者训练营</Tag><ExperimentOutlined className="today-icon"/><h2>{states[2] ? "今日训练已完成" : "抽卡，还是给橡皮鸭讲课？"}</h2><p>随机练一项技能，或者把一个概念真正讲明白。</p><Button type="primary" onClick={() => onNavigate("training")}>{states[2] ? "再练一轮" : "进入训练营"} <RightOutlined/></Button></Card>
    </div>
